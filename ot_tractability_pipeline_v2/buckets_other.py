...
from ot_tractability_pipeline_v2.queries_other import *

DATA_PATH = pkg_resources.resource_filename('ot_tractability_pipeline_v2', 'data/')

class Other_buckets(object):
    '''
    ...
    def _process_protein_complexes(self):
        ...
        if self.store_fetched: 
            binding_subunit.to_csv("{}/other_chembl_binding_subunit.csv".format(self.store_fetched))
    ...
    def _assign_buckets_1_to_3(self):
        ...
        other_info = pd.read_sql_query(chembl_clinical_other, self.engine)
        self.all_chembl_targets = self.all_chembl_targets.merge(other_info, how='left', on='parent_molregno')

        if self.store_fetched: 
            self.all_chembl_targets.to_csv("{}/other_all_chembl_targets.csv".format(self.store_fetched))
        ...
        self.out_df = self.all_chembl_targets.merge(self.out_df, how='outer', on='accession', suffixes=('', '_other'))
        ...
        self.out_df['Bucket_1_other'] = 0
        self.out_df['Bucket_2_other'] = 0
        self.out_df['Bucket_3_other'] = 0

        self.out_df.loc[(self.out_df['max_phase_for_ind'] == 4), 'Bucket_1_other'] = 1
        self.out_df.loc[
            (self.out_df['max_phase_for_ind'] < 4) & (self.out_df['max_phase_for_ind'] >= 2), 'Bucket_2_other'] = 1
        self.out_df.loc[
            (self.out_df['max_phase_for_ind'] < 2) & (self.out_df['max_phase_for_ind'] > 0), 'Bucket_3_other'] = 1
    ...
    def _clinical_precedence(self, s):
        return 1 * s['Bucket_1_other'] + 0.7 * s['Bucket_2_other'] + 0.2 * s['Bucket_3_other']
    ...
    def _summarise_buckets(self):
        ...
        self.out_df['Top_bucket_other'] = 10
        for x in range(3, 0, -1):
            self.out_df.loc[(self.out_df['Bucket_{}_other'.format(x)] == 1), 'Top_bucket_other'] = x
            self.out_df['Bucket_{}_other'.format(x)].fillna(0, inplace=True)

        self.out_df['Bucket_sum_other'] = self.out_df['Bucket_1_other'] + self.out_df['Bucket_2_other'] + self.out_df['Bucket_3_other']
    ...
    def assign_buckets(self):
        ...
        self.out_df = self.out_df[['accession', 'symbol',
                                   ...
                                   'Bucket_1_other', 'Bucket_2_other', 'Bucket_3_other',
                                   'Bucket_sum_other', 'Top_bucket_other'
                                   ]]
        ...
        self.out_df['Clinical_Precedence_other'] = self.out_df.apply(self._clinical_precedence, axis=1)

        self.out_df['Category_other'] = 'Unknown'

        self.out_df.loc[(self.out_df['Top_bucket_other'] <= 3), 'Category_other'] = 'Clinical_Precedence'
#        self.out_df.loc[(self.out_df['Top_bucket_other'] == 4) | (self.out_df['Top_bucket_other'] == 5),
#                        'Category_other'] = 'Predicted_Tractable__High_confidence'
#        self.out_df.loc[(self.out_df['Top_bucket_other'] == 6) | (self.out_df['Top_bucket_other'] == 7) | (
#            self.out_df['Top_bucket_other'] == 8) | (self.out_df['Top_bucket_other'] == 9),
#            'Category_other'] = 'Predicted_Tractable__Medium_to_low_confidence'

        self.out_df = self.out_df[(self.out_df['Top_bucket'] < 9 ) | (self.out_df['Top_bucket_ab'] < 10) | (self.out_df['Top_bucket_other'] < 10) ]
